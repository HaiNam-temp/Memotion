...
from .synchronizer import MotionPhase
from .dtw_analysis import DTWResult
...
class HealthScorer:
    ...
    # Public methods for real-time scoring
    def calculate_rom_score(self, angles: Dict[str, float]) -> Dict[str, float]:
        """
        Calculate ROM scores for current angles.
        
        Args:
            angles: Dict of current joint angles.
            
        Returns:
            Dict of ROM scores for each joint.
        """
        rom_scores = {}
        # For arm exercises, use elbow and shoulder angles
        if 'left_elbow' in angles:
            # Use a simple ROM calculation based on angle achieved
            rom_scores['left_elbow'] = min(100.0, (angles['left_elbow'] / 90.0) * 100)  # Assuming 90 degrees is target
        if 'right_elbow' in angles:
            rom_scores['right_elbow'] = min(100.0, (angles['right_elbow'] / 90.0) * 100)
        if 'left_shoulder' in angles:
            rom_scores['left_shoulder'] = min(100.0, (angles['left_shoulder'] / 180.0) * 100)  # Assuming 180 degrees is target
        if 'right_shoulder' in angles:
            rom_scores['right_shoulder'] = min(100.0, (angles['right_shoulder'] / 180.0) * 100)
        
        return rom_scores

    def calculate_stability_score(self, landmarks: Any, recent_frames: List[Dict]) -> Dict[str, float]:
        """
        Calculate stability scores based on landmark consistency.
        
        Args:
            landmarks: Current landmarks.
            recent_frames: Recent frames with landmark data.
            
        Returns:
            Dict of stability scores.
        """
        if len(recent_frames) < 5:
            return {'overall': 50.0}
        
        # Simple stability calculation based on landmark variance
        stability_score = 80.0  # Placeholder - in real implementation would calculate variance
        return {'overall': stability_score}

    def calculate_flow_score(self, dtw_result: Any = None) -> Dict[str, float]:
        """
        Calculate flow score based on DTW results.
        ...
        Returns:
            Dict of flow scores.
        """
        # Placeholder - would use DTW distance for flow calculation
        return {'overall': 75.0}

    def calculate_symmetry_score(self, angles: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        if 'left_elbow' in angles and 'right_elbow' in angles:
            ...
            # Perfect symmetry would be equal angles
            symmetry_diff = abs(left_elbow - right_elbow)
            symmetry_scores['elbows'] = max(0, 100 - symmetry_diff * 2)  # 2 degrees difference = 1 point penalty
        ...
        if 'left_shoulder' in angles and 'right_shoulder' in angles:
            ...
            symmetry_diff = abs(left_shoulder - right_shoulder)
            symmetry_scores['shoulders'] = max(0, 100 - symmetry_diff)
        
        # Overall symmetry
        if symmetry_scores:
            symmetry_scores['overall'] = sum(symmetry_scores.values()) / len(symmetry_scores)
        else:
            symmetry_scores['overall'] = 100.0
            
        return symmetry_scores

    def assess_fatigue(self, frames: List[Dict]) -> str:
        """
        Assess fatigue level based on frame data.
        ...
        # Simple fatigue assessment based on frame count and consistency
    ...
    def generate_recommendations(self, scores: Dict) -> List[str]:
        ...
        if scores.get('rom', {}).get('overall', 100) < 70:
            recommendations.append("Try to achieve a fuller range of motion in your movements.")
        
        if scores.get('stability', {}).get('overall', 100) < 70:
            recommendations.append("Focus on maintaining steady positions without shaking.")
        
        if scores.get('symmetry', {}).get('overall', 100) < 80:
            recommendations.append("Work on balancing movements between left and right sides.")
    ...
    def calculate_overall_score(self, scores: Dict[str, Dict[str, float]]) -> Dict[str, float]:
        """
        Calculate overall score by combining all individual scores.
        
        Args:
            scores: Dict containing rom, stability, flow, and symmetry scores.
            
        Returns:
            Dict with overall score and individual component scores.
        """
        # Extract individual scores
        rom_overall = scores.get('rom', {}).get('overall', 0)
        stability_overall = scores.get('stability', {}).get('overall', 0)
        flow_overall = scores.get('flow', {}).get('overall_flow', 0)
        symmetry_overall = scores.get('symmetry', {}).get('overall', 0)
        
        # Calculate weighted overall score
        overall_score = (
            0.4 * rom_overall +      # ROM is most important
            0.25 * stability_overall +  # Stability is important
            0.2 * flow_overall +     # Flow matters
            0.15 * symmetry_overall   # Symmetry is least weighted
        )
        
        return {
            'overall_score': round(overall_score, 1),
            'rom_score': round(rom_overall, 1),
            'stability_score': round(stability_overall, 1),
            'flow_score': round(flow_overall, 1),
            'symmetry_score': round(symmetry_overall, 1)
        }
